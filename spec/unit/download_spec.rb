...
describe 'java_se::_download_java' do
  context 'default' do
    let(:chef_run) do
      ChefSpec::SoloRunner.new(file_cache_path: '/var/chef/cache', platform: 'centos', version: '7.0') do |node|
        ...
        node.set['java_se']['jdk_version'] = '8u51'
  ...
  context 'url_dir' do
    ...
    it 'fetches java' do
      expect(chef_run).to run_ruby_block(
        'fetch https://example.com/path/to/jdks//jdk-8u51-linux-x64.tar.gz')
    end
  ...
  context 'file_dir' do
    ...
    it 'does not fetch java' do
      expect(chef_run).to_not run_ruby_block(
        'fetch /var/chef/cache/jdks//jdk-8u51-linux-x64.tar.gz')
    end

    it 'validates java' do
      expect(chef_run).to run_ruby_block('validate /var/chef/cache/jdks//jdk-8u51-linux-x64.tar.gz')
    end
  end
